...
`lib/transform.mjs` (which mirror the template's `gradle/rename-mod.gradle` task). No
template is bundled in the package, so `npx` always gets the latest template.
