...
/**
 * Rewrite a resource file/dir name: the assets/examplemod namespace folder, the mixin
 * config, and id-prefixed files like example_ponder.nbt (-> <id>_ponder.nbt).
 */
export function transformResourceName(s: string, n: Names): string {
  return s
    .split('examplemod').join(n.modId)
    .replace(/Example(?=[A-Z])/g, n.pascal)
    .split('example_').join(`${n.modId}_`);
...
export async function applyRename({ targetDir, names }: { targetDir: string; names: Names }): Promise<Names> {
  ...
  // 5. Rename resource files (mixin config, id-prefixed files like the ponder schematic)
  //    and the namespace directory (assets/examplemod -> assets/<id>).
  const resources = path.join(targetDir, 'src', 'main', 'resources');
  if (await pathExists(resources)) {
    // Files first, while they still live inside the examplemod namespace folder. Binary
    // resources (.nbt, .png) are renamed but never have their contents touched.
    for (const file of await walk(resources)) {
      const base = path.basename(file);
      const renamed = transformResourceName(base, names);
      if (renamed !== base) await fs.rename(file, path.join(path.dirname(file), renamed));
    }
    // Then the namespace dirs themselves (assets/examplemod, data/examplemod).
    for (const ns of ['assets', 'data']) {
      const oldNs = path.join(resources, ns, 'examplemod');
      if (await pathExists(oldNs)) await fs.rename(oldNs, path.join(resources, ns, names.modId));
    }
  }
